...
function runBox2D3(workerPort: ThreadWorkerGlobalScope): void {
  ...
  for (let i = 0; i < loop10; i++) {
    ...
    for (let j = 0; j < loop5; j++) {
      ...
      if (j > 2) {
        ...
        body.applyForceToCenter(new B2Vec2((i + 1) * (j + 1) * 2000, 0), true);
      }
      if (i > 2) {
        body.applyForceToCenter(new B2Vec2(0, -(i + 1) * (j + 1) * 500), false);
      } else {
        body.applyForceToCenter(new B2Vec2(0, (i + 1) * (j + 1) * 5000), true);
      }
      addEqual(y, deltaY);
    }
    addEqual(x, deltaX);
